...
int pktloss::batchCrossEntropyLoss(pktmat& lossMat, pktmat& yMat, pktmat& yHatMat) {
    ...
    for (int r = 0; r < yMat.rows(); ++r) {
        ...
        for (int c = 0; c < yMat.cols(); ++c) {
            ...
            // //Normalize the output matrix and golden matrix
            // yHatMat.normalizeMinMax();
            // yMat.normalizeMinMax();

            int yHatElem = yHatMat.getElem(r, c);
            int yMatElem = yMat.getElem(r, c);
            std::cout << "row = " << r << "\n";
            std::cout << "column = " << c << "\n";
            std::cout << "yHatElem = " << yHatElem << "\n";
            ...
            if (yMat.getElem(r, c) == 1) {
                ...
                //columnLoss -= (/*PKT_MAX **/ intRoundLog(2, minVal(yHatElem, PKT_MAX), 0, yShift));
                columnLoss += ((yHatElem - PKT_MAX) * (yHatElem - PKT_MAX)) / 2;
                std::cout << "yMatElem == 1, columnLoss = " << columnLoss << "\n";
            }
            else {
                //columnLoss += (yHatElem * yHatElem) / 2;
                columnLoss += ((-yMatElem * std::log(yHatElem)) - ((1-yMatElem) * std::log(1 - yHatElem)));
                std::cout << "yMatElem != 1, -yMatElem * log(yHatElem) = " << (-yMatElem * std::log(yHatElem)) << "\n";
                std::cout << "yMatElem != 1, ((1-yMatElem) * log(1 - yHatElem)) = " << ((1-yMatElem) * std::log(1 - yHatElem)) << "\n";
                std::cout << "yMatElem != 1, columnLoss = " << columnLoss << "\n";
            }
        }
        lossMat.setElem(r, 0, columnLoss);
